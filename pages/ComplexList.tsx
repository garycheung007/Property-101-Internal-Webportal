                                            ...
                                            return (
                                                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                                                    ...
                                                    {(() => {
                                                        ...
                                                        const responseOverdue = !meetingPast && !meetingForm.nomIssued && !!meetingForm.noiResponseDueDate && isPast(meetingForm.noiResponseDueDate);
                                                        return (
                                                            <div className={`p-3 rounded-xl border ${responseOverdue ? 'bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-800/50' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700'}`}>
                                                                <div className={`text-[9px] font-bold uppercase tracking-widest mb-1.5 ${responseOverdue ? 'text-red-500' : 'text-slate-400'}`}>NOI Response Due</div>
                                        ...
                                        <div className="bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 shadow-sm overflow-hidden">
                                            ...
                                            {meetingForm.date && meetingForm.type !== 'Committee' && (() => {
                                                ...
                                                const chips = [
                                                    ...
                                                    { label: 'NOI Response Due', date: meetingForm.noiResponseDueDate || '—', done: !!meetingForm.nomIssued, status: meetingForm.nomIssued ? '✓ NOM issued' : meetingForm.noiResponseDueDate ? (!meetingPast && isPast(meetingForm.noiResponseDueDate) ? 'Overdue' : meetingPast ? 'Closed' : `${daysUntil(meetingForm.noiResponseDueDate)}d away`) : 'Not set', color: meetingForm.nomIssued ? 'emerald' : meetingForm.noiResponseDueDate && !meetingPast && isPast(meetingForm.noiResponseDueDate) ? 'red' : 'slate' },
